...
import type { ServiceAccount } from "~/features/service-accounts/api/client";
...
} from "~/features/service-accounts/api/queries";
import {
  AdminError,
  AdminLayout,
  AdminStatus,
...
export function ServiceAccountsAdminPage() {
  ...
  async function refresh(message?: string, accountId?: string) {
    await queryClient.invalidateQueries({ queryKey: serviceAccountQueryKeys.all });
    if (accountId) {
      await queryClient.invalidateQueries({
        queryKey: serviceAccountQueryKeys.credentials(accountId),
      });
    }
